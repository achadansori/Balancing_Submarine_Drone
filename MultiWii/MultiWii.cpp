  ...
  if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
          ...
          if (conf.activate[BOXARM] == 0 && rcSticks == THR_CE + YAW_CE + PIT_CE + ROL_LO) go_disarm();    // Disarm via ROLL
        #endif
        ...
        #ifdef ALLOW_ARM_DISARM_VIA_TX_YAW
          else if (conf.activate[BOXARM] == 0 && rcSticks == THR_CE + YAW_HI + PIT_CE + ROL_CE) go_arm();      // Arm via YAW
        #endif
        #ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
          else if (conf.activate[BOXARM] == 0 && rcSticks == THR_CE + YAW_CE + PIT_CE + ROL_HI) go_arm();      // Arm via ROLL
        #endif
        #ifdef LCD_TELEMETRY_AUTO
